...
<section class="card"><h2>Route</h2><form method="post"><input name="routename" placeholder="Route name" required><select name="vehicleid"><option value="">Choose vehicle</option><?php foreach($vehicles as $vehicle){ ?><option value="<?php echo tm($vehicle['vehicleid']); ?>"><?php echo tm($vehicle['vehiclename'].' — '.$vehicle['registrationno']); ?></option><?php } ?></select><input type="number" name="fee" min="0" step="0.01" placeholder="Transport fee per term (GHS)"><button name="save_route">Save Route</button></form></section>
